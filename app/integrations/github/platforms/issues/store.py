...
class IssueStore:
    ...
    def active_keys(self) -> set[tuple[str, str, int]]:
        """Return (org, repo, number) tuples for issues in the root directory only.

        Issues in synced/ are excluded — they no longer require attention.
        """
    ...
    def move_to_synced(self, org: str, repo: str, number: int, **fields) -> Path | None:
        """Move an issue note from the root to synced/ (no longer requires attention)."""
        filename = self._filename(org, repo, number)
        ...
        log.info("Moved issue %s/%s#%d to synced/", org, repo, number)
        return dest_path

    def restore_to_active(self, org: str, repo: str, number: int) -> Path | None:
        """Move an issue note from synced/ back to root (issue requires attention again)."""
        ...
        log.info("Restored issue %s/%s#%d from synced/ to active", org, repo, number)
        return dest
