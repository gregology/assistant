...
def handle(task: dict):
    log.info("update_github_prs: starting")
    client = GitHubClient()
    ...
    for pr in store.unclassified():
        queue.enqueue({
            "type": "classify_github_pr",
            "org": pr["org"],
            "repo": pr["repo"],
            "number": pr["number"],
        }, priority=6)
        log.info("update_github_prs: queued classify for %s/%s#%d",
                 pr["org"], pr["repo"], pr["number"])

    log.info("update_github_prs: done")


def _refresh_existing(store: PullRequestStore, client: GitHubClient) -> None:
    existing = store.all()
    log.info("update_github_prs: refreshing %d existing PRs", len(existing))
    ...
    for pr in existing:
        ...
        try:
            ...
            log.exception("update_github_prs: failed to fetch %s/%s#%d", org, repo, number)
            continue

        if current["status"] in ("closed", "merged"):
            store.archive(org, repo, number, status=current["status"])
            log.info("update_github_prs: archived %s/%s#%d (%s)",
...
def _discover_new(store: PullRequestStore, client: GitHubClient) -> None:
    ...
    log.info("update_github_prs: found %d assigned PRs via API", len(assigned))
    ...
    for pr in assigned:
        ...
        if key not in known:
            store.save(pr)
            new_count += 1
            log.info("update_github_prs: saved new PR %s/%s#%d",
                     pr["org"], pr["repo"], pr["number"])

    log.info("update_github_prs: created %d new PR files", new_count)
